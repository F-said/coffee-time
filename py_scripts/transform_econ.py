...
def main():
    ...
    for country, _ in countries.items():
        ...
        econ_source = os.path.join(csv_outpath_econ, country, "br_unemp.csv")
        # get all csvs for production
        ecsv = pd.read_csv(econ_source)
        
        # compute average for each row
        new_df = ecsv.copy()
        new_df["avg_unemp_thou"] = new_df.iloc[:, 2:6].mean(axis=1)
        
        new_df.drop(columns=["1st_quart_unemp_thou", "2nd_quart_unemp_thou", "3rd_quart_unemp_thou", "4th_quart_unemp_thou"], inplace=True)
